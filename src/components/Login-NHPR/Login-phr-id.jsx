...
const LoginPHRID = () => {
   ...
   */
    ...
    const [NHPRloginType, setNHPRLoginType] = useState(1);
    const handeNHPRLoginType = (e) => {
        const value = Number(e.target.value);
        switch (value) {
            case 1:
                setNHPRLoginType(value);
                break;
            case 2:
                setNHPRLoginType(value);
                break;
    ...
    return (
        ...
        <>
            ...
            <div className="flow-feilds aadharCardLogin">
                ...
                {!showOtpBox && (
                    <div className="feilds">
                        ...
                        <input type="text" className="filds-form" placeholder="9870-6540-3210"
                            value={formateAadhar}
                            autoComplete="off"
                            onChange={handleFormateAadhar} />
                        <p className="forgotHealthCare">Forgot Healthcare Professional ID/Username?</p>
                    </div>
                )}
                {showOtpBox && (
                    <div className="feilds">
            ...
            <div className='selectLoginType'>
                <label for="Password" className={`selectTypeLogin ${NHPRloginType === 1 ? 'active-NHPRlogin' : ''}`}>
                    <input type='radio' name='loginType' id='Password' onClick={handeNHPRLoginType} value={1} />
                    <div className='loginTypeSelectOne'>
                        <img src={Password} />
                        <p>Password</p>
                    </div>
                </label>
                <label for="aadharOTP" className={`selectTypeLogin ${NHPRloginType === 2 ? 'active-NHPRlogin' : ''}`}>
                    <input type='radio' name='loginType' id='aadharOTP' onClick={handeNHPRLoginType} value={2} />
                    <div className='loginTypeSelectOne'>
                        <img src={AadharId} />
                        <p>Aadhar OTP</p>
                    </div>
                </label>
                <label for="MobileOTP" className={`selectTypeLogin ${NHPRloginType === 3 ? 'active-NHPRlogin' : ''}`}>
                    <input type='radio' name='loginType' id='MobileOTP' onClick={handeNHPRLoginType} value={3} />
                    <div className='loginTypeSelectOne'>
                        <img src={MobileNHPR} />
                        <p>Mobile OTP</p>
                    </div>
                ...
                <div className="flow-feilds">
                    <div className="feilds">
                        ...
                        <input type="text" className="filds-form" placeholder="9870-6540-3210"
                            ...
                            autoComplete="off"
                            onChange={handleFormateAadhar} />
                        <p className="donthaveRecivedOtp">Didn't receive OTP? <span>Resend OTP 78 seconds remaining</span></p>
                    </div>
                </div>
            )}
            ...
            <div className="flow-feilds" style={{ minHeight: 'auto' }}>
                <div className="feilds feildsBtn">
                    ...
                    <button className={`custBtn ${(formateAadhar.length == 14 && isChecked) ? 'btnSuccess' : 'disabled'}`} disabled={formateAadhar.length !== 14 || !isChecked} onClick={!showOtpBox ? handleSendOtp : verifyOtp}> {!showOtpBox ? 'Login' : 'Verify'} <i className="material-icons">arrow_right_alt</i></button>
                </div>
            </div>
        </>
    )
